...
$user_info = $connection->get('users/show', ['screen_name'=> $screen_name]);    
    ...
    // $my_id_str = $userinfo->id;
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
          $delete = new Delete();
          $delete ->Delete($user_id,$connection);
        }
...
<html lang="en">
  ...
  <?php endif; ?>
       ...
       <p>あなたが登録した人</p>
        ...
        if($user_lover){
          ...
          if($your_lover_id[$i]&&$your_lover_img[$i]){
            echo"<tr><td><img src ='".$your_lover_img[$i]."'></td><td>@".$your_lover_id[$i]."</td></tr>";
          }
         }
        }else{
       ...
       <p>気になる人が登録されています。あなたが登録した人があなたを登録すると、お二人にDMでおしらせします。</p>
     </div> 
     <form action="" method="POST">
        ...
        <button type="submit" onclick="document.getElementById('signup').submit();">削除</button>
         <?php
         if(!empty($error['id'])){
           echo $error['id'];
         }
