...
//フォロワーのIDを取得
$followersids = $connection->get('followers/ids',['screen_name'=> '@weatherbot777']);
$followers_ids = $followersids->ids;
//IDを１００人ごとに分割
$followers_ids_divide = array_chunk($followers_ids,100);
//１００人の束がいくつできるか数える
$count_followers_ids_divide = count($followers_ids_divide);
...
<html lang="en">
  ...
  <?php endif; ?>
	    ...
	    <div>
         ...
         <?php
         //トータルページ数
         $totalPage = $count_followers_ids_divide;
        ...
        if($page){
          $followers_ids100 = $followers_ids_divide[$page-1];
          $followers_ids100 = (implode(",", $followers_ids100) . "\n");
          //フォロワーの情報を取得
          $followersinfo = $connection->get('users/lookup',['user_id'=>$followers_ids100]);
         for($i=0; $i<100; $i++){
          $followers_name[$i] = $followersinfo[$i]->name;
          $followers_id[$i] = $followersinfo[$i]->screen_name;
          $followers_img[$i] = $followersinfo[$i]->profile_image_url_https;
          if($followers_name[$i]&&$followers_id[$i]&&$followers_img[$i]){
            echo"<tr><td><img src ='".$followers_img[$i]."'></td><td>".$followers_name[$i]."</td><td>@".$followers_id[$i]."</td></tr>";
          }
         }
        }
         ?>
